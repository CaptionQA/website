...
def calculate_category_scores(domain, prompt, model_file):
    ...
    category_scores = defaultdict(lambda: {"score": 0, "total": 0})
    overall_score_sum = 0
    overall_total = 0
    ...
    for image_id, questions in data.items():
        for q in questions:
            overall_total += 1
            # Use the score field which includes partial credit for "Cannot answer"
            ...
            if "category" in q and len(q["category"]) > 0:
                ...
                category_scores[top_category]["score"] += score

    # Calculate percentages
    results = {}
    for cat, scores in category_scores.items():
        if scores["total"] > 0:
            results[cat] = round(100 * scores["score"] / scores["total"], 2)

    # Calculate overall score
    overall_score = round(100 * overall_score_sum / overall_total, 2) if overall_total > 0 else None
